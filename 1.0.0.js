// ==UserScript==
// @name         淘宝和天猫通用商铺商品信息抓取(单页)
// @namespace    http://tampermonkey.net/
// @version      1.0.0
// @description  以img为中心抓取；(目前仅支持大图/图表模式)仅抓取店铺信息，商品主图及信息；不抓取略缩图、页面左侧推荐、本店内推荐
// @description  [淘宝] [天猫] [taobao] [tmall]
// @author       凡云 <2708799299@qq.com>
// @compatible   chrome
...
function returnData(list){
    ...
    if (list){
        ...
        console.log(json);
        msg = "抓取完毕，请查看console控制台，建议使用格式工具美化\r确认可直接下载json文件";
    }else{
        msg = "抓取失败，找不到有效列表";
        alert(msg);
        return false;
...
function getInfoTaobao(){
	// var shopname = String($(".shop-name-link").html()).replace(/[\r\n]/g,"").replace(/\ +/g,"");
    // 商铺名称(移除回车换行和空格)
    ...
    // 搜索关键字
    var keyword = $('[name="keyword"]').val()?$('[name="keyword"]').val():"";
    var info = [{
    	"shopname" 	: shopname,
        "type"      : "taobao",
    	"pageNo" 	: pageNo,
...
function getInfoTmall(){
    ...
    // 当前页数
    var pageNo = $(".ui-page-s-len").html().split("/")[0];
    // 总页数
    var pageAll = $(".ui-page-s-len").html().split("/")[1];
    // 获取当前排序类型
    ...
    var keyword = $('[name="keyword"]').val()?$('[name="keyword"]').val():"";
    // 是否为商铺VIP商品
    var vip = getQueryVariable("vip")?getQueryVariable("vip"):"false";
    // 将所有信息组合成数组
    var info = [{
    	"shopname" 	: shopname,
        "type"      : "tmall",
